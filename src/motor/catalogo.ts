...
export function semillasPorSiembra(slug: string, enAlmacigo: boolean): number {
  const sp = ESPECIES[slug]; if (VEGETATIVAS.includes(slug)) return 1; if (enAlmacigo) return 6;
  return sp.familia === 'leguminosa' || sp.familia === 'cucurbitacea' || slug === 'choclo' || slug === 'girasol' ? 3 : 4;
}
export const RALEO_SE_COME = 'rabanito zanahoria remolacha nabo lechuga rucula espinaca acelga kale cebolla-de-verdeo perejil cilantro'.split(' ');
export const BIENALES = 'zanahoria remolacha acelga cebolla puerro perejil apio repollo kale nabo repollitos-de-bruselas'.split(' ');
