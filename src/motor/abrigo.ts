...
export const riesgoHelada = (E: Estado, zona: ZonaId): number => Math.round(clamp(phi((3 - abrigo(E, zona).grados - E.prox.pron.tmin) / 2.2), 0, 1) * 100);
export function enRiesgo(E: Estado, zona: ZonaId): string[] {
  const out: string[] = [];
  for (const id in E.plantas) { const pl = E.plantas[id], sp = ESPECIES[pl.slug]; if (E.celdas[pl.celda].zona === zona && pl.etapa !== 'semilla' && (sp.helada === 'muere' || sp.helada === 'sensible') && !out.includes(sp.nombre)) out.push(sp.nombre); }
  return out;
}
