...
export function sueloDeCelda(E: Estado, celda: CeldaId): CategoriaSuelo { const base = sueloBase(E, celda); return E.celdas[celda].mo >= 80 && base === 'FRANCO_FERTIL' ? 'HUMEDO_RICO' : base; }
...
export function fMaceta(E: Pick<Estado, 'patio'>, sp: Especie, celda: CeldaId): number {
  const m = macetaDe(E, celda); if (!m) return 1;
...
export const bajoTunel = (E: Pick<Estado, 'celdas' | 'tunel'>, celda: CeldaId): boolean => !!E.tunel[E.celdas[celda].zona];
...
export function humedad(E: Estado, celda: CeldaId, w: Pick<Tiempo, 'lluvia' | 'tmax'>): number {
  const c = E.celdas[celda], z = zonaDe(E, celda), m = macetaDe(E, celda);
...
export function tempEfectiva(E: Estado, celda: CeldaId, w: Pick<Tiempo, 'tmed'>): number { return w.tmed + (zonaDe(E, celda).calor || 0) + (bajoTunel(E, celda) ? 3 : 0); }
...
export function factoresPlanta(E: Estado, pl: Planta, w: Tiempo = E.prox.real): Factores {
  ...
  return {
    ...
    suelo: { f: clamp(fSuelo(E, sp, pl.celda) * fMaceta(E, sp, pl.celda), 0, 1), mo: Math.round(E.celdas[pl.celda].mo), maceta: fMaceta(E, sp, pl.celda) },
    vecinos: ve,
  };
}
