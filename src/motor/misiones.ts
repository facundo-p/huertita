...
export interface Mision { id: string; titulo: string; texto: string; premio: Record<string, number> }
...
export function cumplir(E: Estado, id: string, evs: Evento[]): void {
  ...
  const m = MISIONES.find((x) => x.id === id); if (!m) return;
  E.misiones[id] = E.turno;
  for (const s in m.premio) E.sobres[s] = (E.sobres[s] || 0) + m.premio[s];
  evs.push(anotar(E, 'logro', 'Logro: ' + m.titulo + '. Una vecina te pasa sobres de ' + Object.keys(m.premio).map((s) => ESPECIES[s].nombre.toLowerCase()).join(', ') + '.'));
}
