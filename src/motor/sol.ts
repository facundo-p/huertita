...
function sombraDe(o: Obstaculo, m: number, px: number, py: number, dx: number, dy: number, pendiente: number, hojas: boolean): number {
  if (o.tipo === 'muro') {
    const ax = o.desde[0] * m, ay = o.desde[1] * m, bx = o.hasta[0] * m, by = o.hasta[1] * m;
    const ex = bx - ax, ey = by - ay, den = dx * ey - dy * ex;
    if (Math.abs(den) < 1e-9) return 0;
    const s = ((ax - px) * ey - (ay - py) * ex) / den, u = ((ax - px) * dy - (ay - py) * dx) / den;
    if (s <= 1e-6 || u < 0 || u > 1) return 0;
    return o.alto > ALTURA_PLANTA + s * pendiente ? (o.opacidad ?? 1) : 0;
  }
  if (o.tipo === 'losa') {
    const s = (o.alto - ALTURA_PLANTA) / pendiente, x = (px + dx * s) / m, y = (py + dy * s) / m;
    return x >= o.desde[0] && x <= o.hasta[0] && y >= o.desde[1] && y <= o.hasta[1] ? 1 : 0;
  }
  const cx = o.en[0] * m - px, cy = o.en[1] * m - py, R = o.copa * m;
  const alCentro = cx * dx + cy * dy, lejos2 = cx * cx + cy * cy - alCentro * alCentro;
  if (lejos2 >= R * R) return 0;
  const media = Math.sqrt(R * R - lejos2), s1 = Math.max(0, alCentro - media), s2 = alCentro + media;
  if (s2 <= 0) return 0;
  const h1 = ALTURA_PLANTA + s1 * pendiente, h2 = ALTURA_PLANTA + s2 * pendiente;
