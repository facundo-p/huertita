...
export interface Caracter { nombre: string; dT: number; lluvia: number; helada: number; texto: string }
...
export function interp(vals: number[], dia: number): number {
  const centros: number[] = []; let acc = 0;
  for (let i = 0; i < 12; i++) { centros.push(acc + 15); acc += DIAS_MES[i]; }
  for (let j = 0; j < 12; j++) {
    const a = centros[j], b = j === 11 ? centros[0] + 365 : centros[j + 1];
...
export const estacionDe = (dec: number): string => (dec >= 34 || dec <= 8 ? 'verano' : dec <= 17 ? 'otoño' : dec <= 26 ? 'invierno' : 'primavera');
...
export function generarTiempo(E: Estado, dec: number): { real: Tiempo; pron: Pronostico } {
  const car = CARACTERES[E.caracter], dia = diaCentral(dec), est = estacionDe(dec);
  ...
  const real: Tiempo = { dec, tmed: r1(tmed), tmax: r1(tmax), tmin: r1(tmin), lluvia: Math.round(lluvia), helada: tmin <= 3, ola: tmax >= 35, estacion: est };
  // El pronóstico es la realidad con ruido: informa, no garantiza.
  const ftmin = tmin + gauss(E) * 1.8, ftmax = tmax + gauss(E) * 1.5;
  const pron: Pronostico = {
    tmin: Math.round(ftmin), tmax: Math.round(ftmax),
    pHelada: Math.round(clamp(phi((3 - ftmin) / 2.2), 0, 1) * 100),
    lluvia: azar(E) < 0.72 ? (lluvia < 12 ? 'seca' : lluvia > 45 ? 'llovedora' : 'normal') : (['seca', 'normal', 'llovedora'] as const)[Math.floor(azar(E) * 3)],
  };
  return { real, pron };
}
