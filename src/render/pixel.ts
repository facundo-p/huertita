...
import * as SP from '../arte/sprites';
  ...
  function geometria(es) {
    ...
    if (cam === 'cerca') {
      ...
      G.H = 256; G.cols = cols; G.filas = ys; G.cw = cw; G.x0 = x0; G.yF = 170; G.yB = 132; G.zona = es.cerca.zona; G.masIzq = i0 > 0; G.masDer = i0 + 4 < xs.length;
  ...
  RenderPixel.prototype._fondo = function (g, es, G) {
    ...
    for (y = 0; y < es.alto; y++) for (x = 0; x < es.ancho; x++) {
      var ch = es.mapa[y][x], X = x * T, Y = G.fy(y), h = G.fh(y);
      if (ch === 'P' || ch === 'H') continue;
      var send = (ch === '.' || ch === 'C') && (y === 3 || y === 6 || x === 4 || (x === 6 && y < 3));
    ...
    else { r(0, yc, G.W, hc, C.casa); r(0, yc, G.W, 11, C.techo); r(0, yc + 11, G.W, 2, C.techo2); for (x = 0; x < G.W; x += 8) { r(x, yc + 2, 5, 7, C.techo2); r(x, yc + 2, 5, 2, C.techo3); } r(200, yc + 14, 16, 18, C.puerta); r(200, yc + 14, 16, 2, '#a8341d'); r(211, yc + 23, 2, 2, C.paja); r(168, yc + 16, 20, 12, '#7fd6e0'); r(177, yc + 16, 2, 12, C.casa2); r(168, yc + 21, 20, 2, C.casa2); r(170, yc + 17, 5, 3, '#c8f4f8'); }
    ...
    Object.keys(es.celdas).forEach(function (k) {
      ...
      if (c.zona === 'macetas') {
        ...
        if (c.zona === 'elevado') { if (c.borde.n) { r(X, Y, T, 3, C.madera2); r(X, Y, T, 1, C.madera3); } if (c.borde.o) { r(X, Y, 3, h, C.madera); r(X, Y, 1, h, C.madera3); } if (c.borde.e) r(X + T - 3, Y, 3, h, C.madera2); if (c.borde.s) { var fa = obl ? 9 : 4; r(X, Y + h - 2, T, fa, C.madera); r(X, Y + h - 2, T, 1, C.madera3); r(X, Y + h + fa - 4, T, 2, C.madera2); if (obl) { r(X + 15, Y + h - 1, 1, fa - 1, C.madera2); r(X, Y + h + fa - 2, T, 2, 'rgba(0,0,0,0.2)'); } } }
    ...
    // compostera
    var qc = G.celda('5,7'), cp = es.compost, ch2 = G.obl ? 24 : 26, cyy = qc.y + qc.h - ch2 - 2;
    ...
    if (es.sombraPared > 0) { g.fillStyle = C.sombra; g.fillRect(0, G.muro, 6.2 * T, Math.round(es.sombraPared * G.TH)); }
  };
  ...
  RenderPixel.prototype._cuadro = function () {
    ...
    claves.forEach(function (k) {
      ...
      var by = (G.cam !== 'cerca' && c.zona === 'macetas' && G.obl) ? q.by - 1 : (c.zona === 'macetas' && G.cam !== 'cerca') ? q.by - 9 : q.by;
      var chico = c.zona === 'almacigo' && G.cam !== 'cerca';
      ...
      if (nn > 1) { // varios plantines juntos: en la bandeja, uno por celdita; en tierra, un manojo apretado
        var enBandeja = c.zona === 'almacigo', paso = enBandeja ? (G.cam === 'cerca' ? 9 : 5) : 6 * esc, ee = enBandeja ? (G.cam === 'cerca' ? 1.4 : 0.7) : esc * 0.78, m = enBandeja ? nn : Math.min(4, nn);
  ...
  RenderPixel.prototype._frente = function (g, es, G, t) {
    var B = SP.pincel(g, 0, 0, 1), r = B.r, q1 = G.celda('7,1'), q2 = G.celda('7,2'), ax = q1.bx, ay = q2.y + q2.h - 4, alto = G.obl ? 58 : 40, sw = Math.round(Math.sin(t * 0.3) * 1.5);
    if (es.arbolConHojas) { g.fillStyle = C.sombra; g.beginPath(); g.ellipse(ax - 22, ay + 2, 44, G.obl ? 20 : 34, 0, 0, 6.3); g.fill(); }
    ...
    if (es.tunel) { var a = G.celda('0,4'), b = G.celda('3,5'), top = a.y - (G.obl ? 20 : 4), bot = b.y + b.h + (G.obl ? 4 : 2); g.fillStyle = 'rgba(255,255,255,0.34)'; g.fillRect(0, top, 4 * T, bot - top); for (var u = 0; u <= 4; u++) { var ux = Math.min(4 * T - 2, u * T); r(ux, top, 2, bot - top, C.blanco); } r(0, top, 4 * T, 2, C.blanco); for (u = 0; u < 4 * T; u += 9) r(u, top + 5 + (u % 4), 5, 1, 'rgba(255,255,255,0.6)'); }
  ...
  // ── cámara de cerca ──
  var PROF = { suelo: 78, elevado: 42, almacigo: 14 }; // px de tierra útil (1 px ≈ 0,75 cm)
  RenderPixel.prototype._fondoCerca = function (g, es, G) {
    var B = SP.pincel(g, 0, 0, 1), r = B.r, cielo = CIELO[es.estacion] || CIELO.primavera, i, z = G.zona, n = G.cols.length, x0 = G.x0, x1 = x0 + n * G.cw;
    ...
    if (z === 'suelo' || z === 'elevado') { r(0, 58, 256, 50, C.ladrillo); for (var f = 0; f < 9; f++) { r(0, 58 + f * 6 + 5, 256, 1, C.junta); for (var x = (f % 2) * 10; x < 256; x += 20) r(x, 58 + f * 6, 1, 5, C.junta); } r(0, 58, 256, 3, C.casa2); if (z === 'suelo') { g.fillStyle = C.sombra; g.fillRect(0, 61, 256, 47); } }
    ...
    r(0, 108, 256, 148, C.pasto); for (i = 0; i < 90; i++) r(Math.floor(ruido(i, 1) * 256), 110 + Math.floor(ruido(i, 2) * 60), 1, 3, i % 2 ? C.pasto2 : C.pasto3);
    ...
    if (z === 'almacigo') { r(x0 - 8, 146, n * G.cw + 16, 8, C.madera); r(x0 - 8, 146, n * G.cw + 16, 2, C.madera3); r(x0 - 2, 154, 6, 102, C.madera2); r(x1 - 4, 154, 6, 102, C.madera2); r(0, 236, 256, 20, '#d9704a'); for (i = 0; i < 256; i += 32) r(i, 236, 1, 20, '#b85a38'); return; }
    // cama de tierra: cara de arriba (dos hileras) y corte al frente
    var xa = z === 'elevado' ? x0 - 6 : 0, xb = z === 'elevado' ? x1 + 6 : 256;
    ...
    this._perfil(g, es, G, xa, xb, 172, PROF[z], z);
  };
  ...
  RenderPixel.prototype._perfil = function (g, es, G, xa, xb, y0, prof, z) {
    ...
    if (z === 'elevado') { r(0, y0 + prof, 256, 256 - y0 - prof, '#6b4a30'); for (i = 0; i < 40; i++) r(Math.floor(ruido(i, 31) * 250), y0 + prof + 3 + Math.floor(ruido(i, 33) * 36), 4, 2, '#5a3c26'); r(0, y0 + prof, 256, 1, C.pasto3); }
    ...
    if (z === 'elevado') { r(xa, 112, 6, prof + 60, C.madera); r(xa, 112, 2, prof + 60, C.madera3); r(xb - 6, 112, 6, prof + 60, C.madera2); r(xa, y0 + prof - 2, xb - xa, 4, C.madera2); for (i = 0; i < 3; i++) r(xa, y0 + 10 + i * 14, 6, 1, C.madera2); }
  ...
  RenderPixel.prototype._raices = function (g, es, G) {
    var self = this, B0 = SP.pincel(g, 0, 0, 1), r = B0.r, t = this.t, z = G.zona;
    G.cols.forEach(function (cx, ci) {
      G.filas.forEach(function (fy, fi) {
        ...
        if (z === 'almacigo') { var X = q.x + 3, W = q.w - 6, t2 = C.tierra[Math.min(3, c.humedo)]; r(X, 134, W, 14, '#2a2869'); r(X, 134, W, 2, '#4a48a0'); r(X + 2, 136, W - 4, 11, mez(t2, '#1e0f08', 0.4)); if (c.planta) SP.raiz(SP.pincel(g, q.bx, 137, 1), c.planta, 9); return; }
        if (frente && c.planta) SP.raiz(SP.pincel(g, q.bx, 173, 2), c.planta, Math.floor(PROF[z] / 2) + (z === 'elevado' ? 14 : 0));
    ...
    if (z === 'almacigo') { for (var ci2 = 0; ci2 < G.cols.length; ci2++) r(G.x0 + ci2 * G.cw + 3, 134, 1, 14, '#4a48a0'); }
    if (G.masIzq) { r(2, 150, 6, 2, C.blanco); r(2, 148, 2, 6, C.blanco); } if (G.masDer) { r(248, 150, 6, 2, C.blanco); r(252, 148, 2, 6, C.blanco); }
    if (z === 'elevado' && es.tunel) { g.fillStyle = 'rgba(255,255,255,0.3)'; g.beginPath(); g.ellipse(128, 172, 126, 100, 0, 3.14, 6.29); g.fill(); g.strokeStyle = C.blanco; g.lineWidth = 2; g.beginPath(); g.ellipse(128, 172, 126, 100, 0, 3.14, 6.29); g.stroke(); }
  ...
  geometria = function (es) { var G = _celdaCerca(es); if (G.cam === 'cerca') { var f = G.celda; G.celda = function (k) { var q = f(k); if (!q) return q; if (G.zona === 'almacigo') { q.by = 137; q.s = 2; } if (G.zona === 'macetas') { q.by = q.frente ? 168 : 126; } return q; }; } return G; };
  ...
  RenderPixel.prototype._clima = function (g, r, es, G, t) {
    var W = G.W, H = G.H, i;
    if (es.compost.tandas > 0 && G.cam !== 'cerca') { var q = G.celda('5,7'); for (i = 0; i < 3; i++) { var u = ((t * 0.12 + i * 0.33) % 1); g.globalAlpha = 0.5 * (1 - u); r(q.bx - 4 + i * 4 + Math.sin(t * 0.3 + i) * 2, q.y + q.h - 28 - u * 14, 3, 3, '#ffffff'); g.globalAlpha = 1; } }
