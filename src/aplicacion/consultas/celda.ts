...
import type { Accion, CeldaId, Especie, Estado, Planta, ZonaDePatio } from '../../dominio';
...
export interface PlantaEnFicha {
  ...
  /** cuántos ratos lleva cada una de esas acciones */
  costos: Partial<Record<AccionDePlanta, number>>;
}

/** Un cuidado del suelo: si va en esta celda, si se puede ahora (sin contar los ratos) y cuánto lleva. */
export interface CuidadoDelSuelo {
  va: boolean;
  se: boolean;
  costo: number;
}
...
export interface FichaDeCelda {
  ...
  ponerMulch: CuidadoDelSuelo;
  /** el botón de compost se muestra donde va aunque no haya dosis: dice cuántas hay */
  compost: CuidadoDelSuelo & { dosis: number };
...
function plantaEnFicha(E: Estado, pl: Planta): PlantaEnFicha {
  ...
  const accion = (a: AccionDePlanta): Accion =>
    a === 'mover' ? { tipo: 'trasplantar', planta: pl.id, celda: pl.celda } : { tipo: a, planta: pl.id };
  const se = (a: AccionDePlanta): boolean =>
    a === 'mover' ? M.puedeMoverse(E, pl) === null : M.puede(E, accion(a), { sinMirarRatos: true }) === null;
  const acciones = ACCIONES.filter(se);
  return {
    pl,
    sp,
    punto,
    ...
    acciones,
    costos: Object.fromEntries(acciones.map((a) => [a, M.costoDe(E, accion(a))])),
  };
}

/** Por defecto un cuidado va donde se puede hacer. */
function cuidado(E: Estado, accion: Accion): CuidadoDelSuelo {
  const se = M.puede(E, accion, { sinMirarRatos: true }) === null;
  return { va: se, se, costo: M.costoDe(E, accion) };
}
...
export function fichaDeCelda(E: Estado, k: CeldaId): FichaDeCelda | null {
  ...
  return {
    ...
    ponerMulch: cuidado(E, { tipo: 'mulch', celda: k }),
    compost: {
      ...cuidado(E, { tipo: 'compost', celda: k }),
      va: M.recibeCompost(E, k),
      dosis: M.dosisDeCompost(E),
    },
