...
export function sueloDeCelda(E: Estado, celda: CeldaId): CategoriaSuelo {
  ...
  return E.celdas[celda].mo >= 80 && base === 'FRANCO_FERTIL' ? 'HUMEDO_RICO' : base;
}
...
export function fLuz(sp: Especie, h: number, tmax: number): number {
  let f =
    h >= sp.hideal
      ? 1
      : h >= sp.hmin
        ? 0.7 + (0.3 * (h - sp.hmin)) / Math.max(0.5, sp.hideal - sp.hmin)
        : 0.7 * Math.pow(h / Math.max(1, sp.hmin), 1.5);
...
export function fSuelo(E: Estado, sp: Especie, celda: CeldaId): number {
  return (SUELO_COMPAT[sp.suelo][sueloDeCelda(E, celda)] || 0.85) * (0.65 + (0.35 * E.celdas[celda].mo) / 100);
}
export function fMaceta(E: Pick<Estado, 'patio'>, sp: Especie, celda: CeldaId): number {
  const m = macetaDe(E, celda);
  if (!m) return 1;
  const l = sp.maceta?.litros_min || 8,
    p = sp.maceta?.profundidad_min_cm || 30; // [SUPUESTO] 8 L / 30 cm si huertapp no dice
...
export function humedad(E: Estado, celda: CeldaId, w: Pick<Tiempo, 'lluvia' | 'tmax'>): number {
  ...
  const dren = z.drenaje + (m && m.litros <= 4 ? 0.2 : 0);
...
export function tempEfectiva(E: Estado, celda: CeldaId, w: Pick<Tiempo, 'tmed'>): number {
  return w.tmed + (zonaDe(E, celda).calor || 0) + (bajoTunel(E, celda) ? 3 : 0);
}
...
export function factoresPlanta(E: Estado, pl: Planta, w: Tiempo = E.prox.real): Factores {
  ...
  return {
    ...
    agua: {
      ...
      pide: sp.riego,
      H: r1(clamp(H, 0, 4.5)),
      lo: DESEO_AGUA[sp.riego] - 0.6,
      hi: DESEO_AGUA[sp.riego] + 0.8,
    },
    temp: {
      f: fTemp(sp, t),
      t: r1(t),
...
export function evaluarCelda(E: Estado, slug: string, celda: CeldaId): Evaluacion | null {
  ...
  if (m < 1)
    razones.push(
      'La maceta le queda chica: pide ' +
        (sp.maceta?.litros_min || 8) +
        ' L y ' +
        (sp.maceta?.profundidad_min_cm || 30) +
        ' cm de hondo.',
    );
  ...
  const p =
    (entra ? 1 : 0) * l * s * m * ve.f * vig * germ * (c.fam === sp.familia ? 0.85 : 1) * (enAlm && !sp.dt ? 0.4 : 1);
