...
export const suelosYCompost: SistemaDelPatio = ({ E, w, ev }) => {
  for (const c of Object.values(E.celdas)) if (c.mulch) c.mo = clamp(c.mo + REGLAS.suelo.moPorMulch, 0, 100);
  ...
  E.visitas += Math.round(floresAbiertas(E) * (w.tmed > POLI.calorDesde ? POLI.visitasConCalor : 1));
};
...
export const ajustarRiego: SistemaDelPatio = ({ E }) => {
  while (ratosLibres(E) < 0) for (const z of idsDeZonas(E)) if (E.riego[z] > 0 && ratosLibres(E) < 0) E.riego[z]--;
};
