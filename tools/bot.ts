...
else for (const patio of patios) for (let s = 1; s <= 8; s++) { const E = jugar(() => jugarUnAnio(M, s, undefined, { patio })); console.log(JSON.stringify({ patio, semilla: s, caracter: E.caracter, ...M.balance(E), vivas: Object.keys(E.plantas).length })); }
