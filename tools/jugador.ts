...
export interface MotorJugable {
  crearPartida(semilla: number): any; despachar(E: any, a: any): { ok: boolean }; pasarDecada(E: any): { dec: number; tipo: string; texto: string }[];
...
export function jugarUnAnio(M: MotorJugable, semilla: number, narrar?: (linea: string) => void): any {
  const E = M.crearPartida(semilla);
  for (let t = 0; t < 36; t++) {
    for (const pl of Object.values<any>(E.plantas)) {
      ...
      if (E.plantas[pl.id] && E.celdas[pl.celda].zona !== 'almacigo' && pl.n > 1) M.despachar(E, { tipo: 'ralear', planta: pl.id });
      let vueltas = 0;
      while (vueltas++ < 6 && E.plantas[pl.id] && E.celdas[pl.celda].zona === 'almacigo' && sp.dt && pl.prog >= sp.dt.min && M.ventana(pl.slug, E.dec, 'trasplante') !== 'fuera') {
        let mejor: { c: string; p: number } | null = null;
        for (const c in E.celdas) if (!E.celdas[c].planta && E.celdas[c].zona !== 'almacigo') { const ev = M.evaluarCelda(E, pl.slug, c)!; if (!mejor || ev.puntaje > mejor.p) mejor = { c, p: ev.puntaje }; }
        if (!mejor || mejor.p < 0.4 || !M.despachar(E, { tipo: 'trasplantar', planta: pl.id, celda: mejor.c }).ok) break;
      }
    }
    if (E.prox.pron.pHelada > 40) for (const z of ['suelo', 'elevado', 'macetas']) M.despachar(E, { tipo: 'manta', zona: z });
    const calor = E.prox.pron.tmax > 29; for (const z of ['suelo', 'elevado', 'macetas']) M.despachar(E, { tipo: 'riego', zona: z, nivel: calor ? 2 : 1 });
    let guarda = 0;
    while (M.ratosLibres(E) > 0 && guarda++ < 10) {
      ...
      for (const s in E.sobres) if (E.sobres[s] > 0 && M.ventana(s, E.dec) !== 'fuera') for (const c in E.celdas) if (!E.celdas[c].planta) {
        const sp = M.ESPECIES[s], alm = E.celdas[c].zona === 'almacigo';
