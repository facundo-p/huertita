...
export function completar(slug: string, r: EspecieRepo): Especie {
  ...
  return {
    ...
    marco: marcoDe(slug, r.grupo), perenne: PERENNES.includes(slug), flor: esFlor || slug === 'lavanda' || slug === 'borraja', fruto: r.grupo === 'Hortaliza de fruto' && slug !== 'choclo',
    sup: [...new Set(sup)],
  };
}
...
const MARCO_CM: Record<string, number> = {
  // hoja
  lechuga: 25, espinaca: 15, acelga: 30, rucula: 8, kale: 40, repollo: 45, radicchio: 30, berro: 15, apio: 25,
  brocoli: 45, coliflor: 45, 'repollitos-de-bruselas': 50, 'cebolla-de-verdeo': 8,
  // raíz y bulbo
  nabo: 10, zanahoria: 6, remolacha: 10, rabanito: 5, papa: 30, batata: 35, cebolla: 10, ajo: 12, puerro: 12,
  // fruto
  tomate: 50, pimiento: 40, 'aji-picante': 40, berenjena: 50, 'zapallito-de-tronco': 80, zapallo: 120,
  pepino: 40, melon: 100, sandia: 120, choclo: 25, frutilla: 25,
  // legumbres
  chaucha: 15, arveja: 8, haba: 20,
  // aromáticas
  albahaca: 25, perejil: 15, cilantro: 15, oregano: 30, tomillo: 25, romero: 60, salvia: 40, menta: 30,
  melisa: 30, ciboulette: 15, laurel: 120, eneldo: 20, lavanda: 60,
  // flores
  calendula: 25, copete: 20, borraja: 40, capuchina: 40, cosmos: 30, girasol: 40,
};
...
const ALTO_M: Record<string, number> = {
  choclo: 2.2, girasol: 2, chaucha: 1.8, pepino: 1.8, tomate: 1.6, laurel: 3, arveja: 1.2, haba: 1.2,
  cosmos: 1.2, eneldo: 1.2, romero: 1.2, berenjena: 1, 'repollitos-de-bruselas': 0.9, pimiento: 0.8,
  'aji-picante': 0.8, kale: 0.8, borraja: 0.8, lavanda: 0.7, puerro: 0.7, brocoli: 0.7, coliflor: 0.6,
  papa: 0.6, ajo: 0.6, salvia: 0.6, melisa: 0.6, 'zapallito-de-tronco': 0.6, apio: 0.5, cilantro: 0.5,
  calendula: 0.5, cebolla: 0.5, 'cebolla-de-verdeo': 0.5, menta: 0.5, acelga: 0.45, nabo: 0.4, zanahoria: 0.4,
  repollo: 0.4, oregano: 0.4, zapallo: 0.4, perejil: 0.35, remolacha: 0.35, batata: 0.3, melon: 0.3, sandia: 0.3,
  radicchio: 0.3, rucula: 0.3, espinaca: 0.3, copete: 0.3, capuchina: 0.3, lechuga: 0.25, tomillo: 0.25,
  frutilla: 0.25, berro: 0.2, rabanito: 0.2,
};
const POR_GRUPO: Record<string, { cm: number; alto: number }> = {
  'Hortaliza de hoja': { cm: 25, alto: 0.35 }, 'Hortaliza de raíz/bulbo': { cm: 10, alto: 0.4 },
  'Hortaliza de fruto': { cm: 50, alto: 1 }, Legumbre: { cm: 15, alto: 1.2 },
  'Aromática': { cm: 30, alto: 0.5 }, 'Flor polinizadora': { cm: 30, alto: 0.6 },
};
...
export function marcoDe(slug: string, grupo: string): Marco {
  ...
  return { cm, huella, alto: ALTO_M[slug] ?? g.alto, porCelda: huella > 1 ? 1 : Math.min(9, Math.max(1, Math.floor(50 / cm) ** 2)) };
}
