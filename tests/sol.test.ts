...
const patio = (obstaculos: Patio['obstaculos'], horizonte = 0): Patio => ({
  ...
  bienvenida: '',
  celdaM: 0.5,
  plano: Array(12).fill('zzzzzzzzzzzz'),
  zonas: [{ ...ZONA, riegoCosto: [0, 1, 2, 3] }],
  obstaculos,
  horizonte,
  sol: 'geometria',
});
const VERANO = 36,
  INVIERNO = 18,
  EQUINOCCIO = 27; // fines de diciembre, fines de junio, fines de septiembre
...
describe('posición del sol a 34,6° sur', () => {
  it('al mediodía está al norte, alto en verano y bajo en invierno', () => {
    const v = posicionSol(355, 12),
      i = posicionSol(172, 12);
    expect(v.norte).toBeGreaterThan(0);
    expect(i.norte).toBeGreaterThan(0);
    expect(Math.abs(v.este)).toBeLessThan(1e-9);
  ...
  it('sale por el este y se pone por el oeste; en verano sale del lado sur', () => {
    expect(posicionSol(80, 8).este).toBeGreaterThan(0);
    expect(posicionSol(80, 16).este).toBeLessThan(0);
    expect(posicionSol(355, 5.5).norte).toBeLessThan(0);
    expect(posicionSol(172, 8).norte).toBeGreaterThan(0);
  });
});
...
describe('horas de sol', () => {
  it('a cielo abierto: unas 14 h en verano (tope 12), unas 10 en invierno, 12 en el equinoccio', () => {
    ...
    expect(horasSolGeometria(p, 5, 5, INVIERNO)).toBeGreaterThan(9.5);
    expect(horasSolGeometria(p, 5, 5, INVIERNO)).toBeLessThan(10.3);
  ...
  it('el horizonte de casas vecinas recorta más en invierno, que el sol anda bajo', () => {
    ...
    expect(dInv).toBeGreaterThan(2.5);
    expect(horasSolGeometria(barrio, 5, 5, INVIERNO)).toBeGreaterThan(5);
  });
  it('un paredón al NORTE sombrea, y mucho más en invierno; uno al SUR casi nada', () => {
    const norte = patio([{ tipo: 'muro', nombre: 'n', desde: [-20, 0], hasta: [32, 0], alto: 2 }]),
      sur = patio([{ tipo: 'muro', nombre: 's', desde: [-20, 12], hasta: [32, 12], alto: 2 }]);
  ...
  it('un muro al este saca el sol de la mañana y uno al oeste el de la tarde, en espejo', () => {
    const este = patio([{ tipo: 'muro', nombre: 'e', desde: [12, -20], hasta: [12, 32], alto: 3 }]),
      oeste = patio([{ tipo: 'muro', nombre: 'o', desde: [0, -20], hasta: [0, 32], alto: 3 }]);
  ...
  it('un árbol caduco sombrea en verano y deja pasar casi todo en invierno', () => {
    const arbol = (caduco: boolean) =>
      patio([{ tipo: 'arbol', nombre: 'a', en: [6, 3], alto: 7, copa: 6, fuste: 2, caduco }]);
    const caduco = arbol(true),
      perenne = arbol(false),
      libre = patio([]);
  ...
  it('un techo (el balcón de arriba) tapa el sol alto del verano y deja entrar el bajo del invierno', () => {
    ...
    const fondoVerano = horasSolGeometria(p, 5, 4, VERANO),
      fondoInvierno = horasSolGeometria(p, 5, 4, INVIERNO);
  ...
  it('una baranda de barrotes sombrea menos que un muro ciego', () => {
    const ciego = patio([{ tipo: 'muro', nombre: 'm', desde: [-20, 0], hasta: [32, 0], alto: 1 }]),
      barrotes = patio([{ tipo: 'muro', nombre: 'b', desde: [-20, 0], hasta: [32, 0], alto: 1, opacidad: 0.4 }]);
