...
const entre = (a: string, b: string) => {
  const i = html.indexOf(a),
    j = html.indexOf(b, i);
  return i < 0 || j < 0 ? '' : html.slice(i + a.length, j);
};
const head = entre('<head>', '</head>'),
  body = entre('<body>', '</body>');
...
writeFileSync(
  'dist-artifact/huertita-artifact.html',
  [titulo, fuentes, estilos, body.trim(), scripts].join('\n') + '\n',
);
